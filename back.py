...
ball = pygame.Surface((20, 20))  # createBallSize
ball.fill((WHITE))  # colorBall
ball_rect = ball.get_rect()  # currentPoint
ball_speed = 5  # movePointSpeed


def create_enemy():
    enemy = pygame.Surface((20, 20))  # createEnemySize
    enemy.fill(GREEN)
...
def create_bonus():
    bonus = pygame.Surface((30, 30))# createBonusSize
    bonus.fill(BLUE)
...
while is_working:
    ...
    # if ball_rect.bottom >= height or ball_rect.top <= 0:  # indicateFieldsTopBottom
    #     ball_speed[1] = - ball_speed[1]  # changeVector
    #     ball.fill(GREEN)
    # if ball_rect.right >= width or ball_rect.left <= 0:  # indicateFieldsLeftRight
    #     ball_speed[0] = - ball_speed[0]
    #     ball.fill(BLUE)

    main_surface.fill((BLACK))  # paintScreenBlack
